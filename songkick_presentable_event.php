...
class SongkickPresentableEvent {
	...
	function to_html($date_color) {
		...
		$html  = '<div>'.$date.'</div>'.'<div><a class="event" href="'.$this->event->uri.'"><span class="event-location">'. $this->event->location->city.'</span>';
		$html .= '<br><span class="venue">' . htmlentities($venue_name, ENT_QUOTES, SONGKICK_I18N_ENCODING) . '</span></a></div>';
